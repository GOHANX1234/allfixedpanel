...
export default function DatabaseBackup() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to load backup files",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Download JSON database files for backup and recovery</p>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">
                  {files.length > 0 
                    ? format(new Date(Math.max(...files.map(f => new Date(f.modified).getTime()))), 'MMM dd, HH:mm')
                    : 'N/A'
                  }
                </p>
              </div>
      ...
      <Card className="bg-background/50 border-border">
        <CardHeader>
          ...
          <CardDescription>
            Individual JSON database files available for download
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">No Backup Files Found</h3>
              <p className="text-sm text-muted-foreground max-w-md">
                There are no JSON database files available for backup. This might indicate an issue with the system.
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start sm:items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs text-muted-foreground">
                          ...
                          <span>Modified {format(new Date(file.modified), 'MMM dd, yyyy HH:mm')}</span>
                        </div>
                      </div>
                    </div>
